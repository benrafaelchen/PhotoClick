...
const getUser = async (req, res) => {
  ...
  try {
    ...
    res.json({ roleID, roleName, ...userResults[0] });
  } catch (error) {
...
const calculateTotalPrice = (quantities, prices, selectedPack) => {
  ...
  const additionalProductsPrice = Object.keys(quantities).reduce(
    (total, productSerial) =>
      total + (quantities[productSerial] * prices[productSerial] || 0),
    0
  );
  ...
  const totalPrice =
    selectedPack.basePrice - discount + additionalProductsPrice;

  console.log("Pack Price After Discount:", selectedPack.basePrice - discount);
...
const createOrder = async (req, res) => {
  const { eventType, packStyle, eventDate, eventTime, place, quantities } =
    req.body;
  ...
  try {
    ...
    const productPackSerials = await getProductPackSerials(); // Retrieve product serials

    // Retrieve the selected pack style using its serial number
    const selectedPack = packStyles[productPackSerials[packStyle]] || {
      basePrice: 0,
      discount: 0,
    };

    // Calculate the total price including additional products
    const totalPrice = calculateTotalPrice(quantities, prices, selectedPack);

    // Check for the next available OrderNumber starting from 1001
    ...
    dbSingleton.query(checkOrderNumberQuery, (err, orderNumberResult) => {
      ...
      const orderQuery = `
        INSERT INTO orders (OrderNumber, EventName, EventSerial, TotalPrice, EventPlace, Email, OrderDate, OrderHour, DateOfEvent, HourOfEvent)
        VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW(), ?, ?)
      `;
      dbSingleton.query(
        orderQuery,
        ...
        (err, orderResult) => {
          ...
          dbSingleton.query(
            ...
            (err, orderItemsResult) => {
              if (err) {
                console.error("Database query failed at orderItemsQuery:", err);
                return res.status(500).json({
                  success: false,
